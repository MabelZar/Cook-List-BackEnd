...
public interface MealProgrammingRepository extends JpaRepository<MealProgrammingEntity, Integer> {
	...
	String CALCULATE_INGREDIENTS_BY_MEAL_ID = "SELECT new com.cooklist.bean.entity.view.MealMeasuredIngredientView("
			+ " CAST(md.id AS integer),"
			+ " CAST(mi.id AS integer),"
			+ " CAST(i.id AS integer),"
			+ " i.name,"
			+ " CAST(mu.id AS integer),"
			+ " mu.name,"
			+ " mu.symbol,"
			+ " CAST(md.quantity AS double)"
			+ " )"
	...
	List<MealMeasuredIngredientView> calculateIngredients(Integer mealId);
